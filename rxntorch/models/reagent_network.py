...
from .attention import AggregateAttention
from .scoring import ReagentScoring


class ReagentNet(nn.Module):
    def __init__(self, depth, afeats_size, bfeats_size, hidden_size):
        super(ReagentNet, self).__init__()
        self.hidden_size = hidden_size
        self.wln = WLNet(depth, afeats_size, bfeats_size, hidden_size)
        self.attention = AggregateAttention(hidden_size)
        self.reagent_scoring = ReagentScoring(hidden_size)

    def forward(self, atom_feats, bond_feats, atom_graph, bond_graph, mask_neis, mask_atoms):
        atom_feats = self.wln(atom_feats, bond_feats, atom_graph, bond_graph, mask_neis, mask_atoms)
        molecule_feats = self.attention(atom_feats)
        reagent_scores = self.reagent_scoring(molecule_feats)
        return reagent_scores


class ReagentTrainer(nn.Module):
    def __init__(self, rxn_net, lr=1e-4, betas=(0.9, 0.999), weight_decay=0.01, with_cuda=True,
                 cuda_devices=None, log_freq=10, grad_clip=None, lr_decay=0.9, lr_steps=10000):
        super(ReagentTrainer, self).__init__()
    ...
    def iterate(self, epoch, data_loader, train=True):
        ...
        for i, data in enumerate(data_loader):
            ...
            reagent_scores = self.model.forward(data['atom_feats'], data['bond_feats'],
                                                    data['atom_graph'], data['bond_graph'], mask_neis, mask_atoms)
            loss = F.binary_cross_entropy_with_logits(reagent_scores, data['bond_labels'], reduction='none')
