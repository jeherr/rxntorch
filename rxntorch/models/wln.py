...
class WLNet(nn.Module):
    def __init__(self, depth, afeats_size, bfeats_size, hidden_size):
        ...
        self.depth = depth
        self.fc1 = Linear(afeats_size, hidden_size)
        self.graph_conv_nei = Linear(hidden_size + bfeats_size, hidden_size)
        self.graph_conv_atom = Linear(hidden_size * 2, hidden_size)
        self.fc2atom_nei = Linear(hidden_size, hidden_size)
        self.fc2bond_nei = Linear(bfeats_size, hidden_size)
    ...
    def forward(self, atom_feats, bond_feats, atom_graph, bond_graph, mask_neis, mask_atoms):
        ...
        for i in range(self.depth):
            ...
            if (i + 1) == self.depth:
                ...
                atom_feats = self_feats * nei_feats
                atom_feats = torch.where(mask_atoms, atom_feats, torch.zeros_like(atom_feats))
            else:
